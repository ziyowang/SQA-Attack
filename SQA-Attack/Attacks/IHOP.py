...
def ihopattack(ndocs, nqr, ntok, nkw, Vexp, Vobs, fexp, fobs, attack_params):
    ...
    def _build_cost_Vol_some_fixed(free_keywords, free_tags, fixed_keywords, fixed_tags):
        cost_vol = -compute_log_binomial_probability_matrix(ndocs, np.diagonal(Vexp)[free_keywords],
                                                            np.diagonal(Vobs)[free_tags] * ndocs)
        for tag, kw in zip(fixed_tags, fixed_keywords):
            cost_vol -= compute_log_binomial_probability_matrix(ndocs, Vexp[kw, free_keywords],
                                                                Vobs[tag, free_tags] * ndocs)
        return cost_vol
    ...
    def compute_coef_matrix(free_keywords, free_tags, fixed_keywords, fixed_tags):
        if attack_params["no_F"] == True & attack_params["no_V"] == False:
            return _build_cost_Vol_some_fixed(free_keywords, free_tags, fixed_keywords, fixed_tags)
        elif attack_params["no_V"] == True & attack_params["no_F"] == False:
            return _build_cost_freq_some_fixed(free_keywords, free_tags, fixed_keywords, fixed_tags)
        return _build_cost_Vol_some_fixed(free_keywords, free_tags, fixed_keywords, fixed_tags) + \
