...
class Sapattacker:
    def __init__(self, sim_kw_trend, real_td_trend, td_num_per_week, sim_v, real_v, real_doc_num, alpha=0.5,
                 countermeasure=None):
        self.sim_kw_trend = sim_kw_trend
        self.real_td_trend = real_td_trend
        # self.sim_kw_doc=sim_kw_doc
    ...
    def attack(self):
        ...
        # C_matrix = np.multiply(Cf, Cv)
        # C_matrix = np.multiply(self.alpha * Cf , (1 - self.alpha) * Cv)
    ...
    def builtCv(self):
        if self.countermeasure == None:
            log_prob_matrix = compute_log_binomial_probability_matrix(self.real_doc_num,
                                                                      self.sim_v / np.sum(self.sim_v), self.real_v)
        elif self.countermeasure == "obf":
            vaux_mod = [prob * (0.999 - 0.01) + 0.01 for prob in self.sim_v]
            log_prob_matrix = compute_log_binomial_probability_matrix(self.real_doc_num, vaux_mod / np.sum(self.sim_v),
                                                                      self.real_v)
        else:
