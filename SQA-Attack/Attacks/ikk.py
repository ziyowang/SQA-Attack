...
def _run_simmulated_annealing(remaining_tags, remaining_keywords, initial_state, m_matrix, m_prime_matrix, unique_flag,
                              initial_temp=200, cooling_rate=0.99999, reject_threshold=500000):
...
def ikkattack(ndocs,nqr,ntok,nkw,Vexp,Vobs,attack_params):

    #naive_flag = attack_params['naive']
    unique_flag = attack_params['unique']
    cooling_rate = attack_params['cooling_rate']
    ...
    #keyword_predictions_for_each_query = [keyword_predictions_for_each_token[token] for token in token_list ]
    print(token_list)
